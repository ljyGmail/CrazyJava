...
public class C_ExtendedObjectPoolFactory {
    ...
    public void initPool() throws Exception {
        for (var name : config.stringPropertyNames()) {
            ...
            // 这就表明是根据value来创建一个对象
            // 定义createObject创建对象，并将对象添加到对象池中
            if (!name.contains("%")) {
    ...
    public void initProperty() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException {
        for (var name : config.stringPropertyNames()) {
            ...
            if (name.contains("%")) {
                ...
                // 通过target的getClass()获取它的实现类对应的Class对象
    ...
    public static void main(String[] args) throws Exception {
        var epf = new C_ExtendedObjectPoolFactory();
        epf.init("data/ch18/4_2_extObj.txt");
        epf.initPool();
        epf.initProperty();
